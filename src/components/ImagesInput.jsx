            ...
            <Carousel className='mx-auto w-[90%]'>
                <CarouselContent>
                    {imageOptions.map((image) => (
                        <CarouselItem key={image} className='basis-1/3' isSelected={selectedImages.includes(image)} onClick={() => handleImageSelect(image)}>
                            <img className='h-[200px] w-full cursor-pointer rounded-md object-cover' src={getImageUrl(image)} alt={`Listing Image Option ${image}`} />
                        </CarouselItem>
